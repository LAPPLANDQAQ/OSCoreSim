...
OSCoreSim/
│
├── CMakeLists.txt              # CMake 构建配置（C++20, MSVC /utf-8, 两个可执行目标）
...
│   │   └── VirtualFileSystem.cpp #  touch/write/read/ls/rm、用户隔离、文件名校验、导入导出
│   │
│   ├── view/                   # 【视图模块】全局可视化
...
└── tests/                      # 测试脚本与单元测试
    ...
    ├── 06_vfs_test.txt         # VFS 验收测试（touch/write/read/rm + 隔离）
